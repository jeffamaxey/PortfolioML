...
def predictions_roc(n_estimators, max_depth, num_period, criterion):
    ...
    x_train, y_train, x_test, y_test = all_data_DNN(df_returns, df_binary, 0)

    rf_ = RandomForestClassifier(n_estimators, criterion, max_depth, n_jobs=-1,
                                min_samples_split=2, min_samples_leaf=1,
                                random_state=10, verbose=1)
    rf_.fit(x_train_auto, y_train)
    y_proba = rf_.predict_proba(x_test_auto)
    ...
    plt.plot(fpr, tpr, label=f'per{0} (area = %0.4f)' % (roc_auc))
    plt.plot([0, 1], [0, 1], 'k--')
    ...
    for tick in df_returns.columns:
        ...
        df_predictions.to_csv(path_p + f'RAF_{args.name_model}_Predictions_{0}th_Period.csv')
...
if __name__ == "__main__":
    ...
    df_auto_train = pd.read_csv(df_auto_train_path, index_col=0)
    df_auto_test = pd.read_csv(df_auto_test_path, index_col=0)

    x_train_auto = np.array(df_auto_train)
    x_test_auto = np.array(df_auto_test)
